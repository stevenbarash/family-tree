...
- **Research-notes UI fully localized; `formatRelative` switched to `Intl.RelativeTimeFormat`** — closes the last reader-visible English islands. Every article page renders `ResearchNotesPanel`, and inside it the per-note display (`by X · 3 days ago`, `(agent)`, `edited 4h ago by Y`, `retracted by Z`), the action buttons' `aria-label`s, the retract confirm prompt, the inline add-note form (placeholders, errors, hint, button), the edit-note form, and the history dialog (event kinds, loading/retry/empty states, `by` author, snapshot disclosure) — all of those were English-hardcoded:
