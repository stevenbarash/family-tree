...
- **Article pages returned HTTP 500 on the auth-on Render deploy (`frontend`)** — #17 moved `/[locale]/[slug]` from `force-dynamic` to on-demand ISR (`export const revalidate = 60`). But the route transitively calls `headers()` — the shared `[locale]` layout hydrates next-intl's client provider — so it can never be statically generated. The ISR tag made Next attempt to populate a route cache anyway, and that cache-fill render threw `DYNAMIC_SERVER_USAGE`, surfacing as a 500 on *every* article page in production (e.g. `/ru/haskel-pinchas-ayzman`). It shipped undetected because `next dev` ignores ISR, a production build silently downgrades the route to dynamic rather than erroring (only `export const dynamic = 'error'` makes it throw, naming `headers()`), and the `static-rendering` guard test inspects source text without rendering. The route is reverted to `force-dynamic` — the pre-#17 behavior, matching every sibling route; the `static-rendering` test now asserts that invariant. The family-tree `unstable_cache` work from #17 is unaffected and stands.
