...
- **Translated titles with YAML-special characters no longer corrupt their own frontmatter (`core` + `cli`)** — `wai i18n sync` wrote `title: ${titleTranslation}` with the model's raw output interpolated unquoted into the translation file's frontmatter, so a title carrying a colon-space (`Иван: судьба`), a leading quote, a `#`, or a stray newline produced YAML that `parsePage` rejects on the next read — and the file was already on disk via `atomicWrite`, so the sync "succeeded" and exited 0 while leaving an unreadable page. The shared `yamlScalar` helper it should have used was itself unsafe: it quoted on specials but escaped only `"`, leaving backslashes (the escape introducer in a double-quoted YAML scalar) and literal newlines/tabs unescaped, so a value like `C:\Users` round-tripped to invalid YAML. `yamlScalar` now escapes `\` first, then `"`, `\n`, `\t`, `\r`, and is exported from `core` so the three i18n-sync title sites (article + both talk-page titles) route through the one correct escaper instead of hand-quoting. Verified by core round-trip tests over adversarial titles and a CLI integration test that drives a YAML-hostile translated title end-to-end.
- **`wai author` verify gate was silently dead; now active with a `warn` severity floor (`cli`)** — the verify phase filters drift findings to the page being authored so pre-existing repo-wide drift doesn't block a new page, but it built the match path as the legacy flat `pages/<slug>.md` while canonical articles live under `pages/en/<slug>.md` (v2 layout). The filter therefore matched nothing, reduced the finding set to `[]`, and the gate (`consistency`/`citation`, blocking on any finding) reported 0 and never fired — drifty drafts logged as clean. Fixing the path alone would over-correct: the corpus carries citation `info` findings ("factual line has no source") on ~52% of pages, so an any-finding gate would wall most re-authoring. The gate now matches `pages/en/` **and** blocks only on `warn`+ severity; `info`-level findings are advisory (a one-line stderr note points at `wai check`). On the current corpus this blocks the 5 pages with genuine infobox/talk-page inconsistencies and none of the citation-nudge pages. Extracted as pure, tested helpers (`findingsForRunSlug`, `atOrAboveSeverity`).
- **`wai interview` loaded no GEDCOM evidence on a v2 data repo (`cli`)** — the evidence loader read the article at the legacy flat `pages/<slug>.md` to extract the derived-record pointer, but canonical articles live under `pages/en/<slug>.md`, so `existsSync` failed and the interview ran with `derived: null` — generating questions with no factual grounding, silently. It now reads `pages/en/<slug>.md`. (The `.narrative.md` sidecar path is unchanged: narrative sidecars are deliberately locale-agnostic top-level files, matching where `wai narrative` writes them.)
