// Note: file extension is .mjs because gedcstruct.js uses ESM syntax.
// Vendored from https://github.com/gedcom7code/js-gedcom
// Author: Luther Tychonievich (technical editor of the GEDCOM 7 spec)
// License: public domain (LICENSE-UNLICENSE in upstream repo)
// Pulled at: 2026-05-17 from upstream commit:
//   9296068bd6a461cdd93f9edc74d8457995897afe
/**
...
class GEDCStruct {
  ...
  fixPtrs(ids, logger) {
    if ('ptr' in this && this.ptr in ids) {
      ...
      // LOCAL PATCH (whoami, 2026-05-17): upstream sets payload=null and
      // deletes ptr, which loses the orphan pointer entirely. Preserve the
      // original `@xref@` string as the payload so downstream parsers can
      // still see the citation even when its target record is missing.
      // This matches the behavior of parse-gedcom@2.x for orphan pointers.
      this.payload = `@${this.ptr}@`
      delete this.ptr
    }
