...
|---|---|---|---|
| ✅ | [`2026-05-22-frontend-performance.md`](./2026-05-22-frontend-performance.md) | Frontend performance — route caching + image optimization | `/[locale]/[slug]` articles → on-demand ISR (drop `force-dynamic`, `revalidate=60`, `generateStaticParams` → `[]`); the shared `getFamilyTree` compute behind `unstable_cache` (tag `gedcom`); page/note/GEDCOM write routes call `revalidatePath`/`revalidateTag` for instant freshness; `next/image` for portraits. Approach A — no `cacheComponents` migration. [spec](../specs/2026-05-22-frontend-performance-design.md). Shipped 2026-05-22; corrected `revalidateTag('gedcom', 'max')` (Next 16's installed type requires the 2nd `profile` arg, an oversight in the plan text). **Article-ISR task reverted 2026-05-31:** `/[locale]/[slug]` transitively calls `headers()` (next-intl client-provider hydration in the shared layout), so it can't be statically generated; `revalidate=60` made Next attempt a route cache and throw `DYNAMIC_SERVER_USAGE` → 500 on every article page on the auth-on Render deploy. Route restored to `force-dynamic`. The other 5 tasks (`getFamilyTree` `unstable_cache`, write-route revalidation, `next/image`) stand. ISR for articles needs the `cacheComponents` migration this plan deferred. |
