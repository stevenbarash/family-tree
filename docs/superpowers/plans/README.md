...
| 🚧 | [`2026-05-20-descope-auth.md`](./2026-05-20-descope-auth.md) | Descope auth + write attribution | `WHOAMI_AUTH`-gated Descope login: redirect-or-fall-through `proxy.ts`, `<AuthProvider>`, `/[locale]/sign-in` flow, `/api/healthz`, API self-gating via `requireSession()`, browser writes attributed to the signed-in family member. Plan 2 of 3 for the Render deployment ([spec](../specs/2026-05-20-render-deployment-design.md) phase 2). |
...
**Total: 52 plans** — 42 shipped (✅), 3 in-progress (🚧), 5 sketches (📝), 2 indexes (🗂), 0 abandoned (📦).
