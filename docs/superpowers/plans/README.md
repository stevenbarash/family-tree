...
|---|---|---|---|
| 📝 | [`2026-05-18-quality-checks-pass-2.md`](./2026-05-18-quality-checks-pass-2.md) | Quality checks Pass 2 | 4 new `wai check` detectors: NAME.TRAN↔title consistency, stale-canonical-sha (translation freshness), title↔infobox-name consistency, pipeline-required frontmatter for translations. Extends the 5-layer defense work that shipped in `feat/gedcom-7-upgrade`. Plan-only — execute in a future session. |
...
**Total: 44 plans** — 38 shipped (✅), 0 in-progress (🚧), 5 sketches (📝), 1 index (🗂), 0 abandoned (📦).
