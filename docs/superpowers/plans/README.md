...
|---|---|---|---|
| ✅ | [`2026-05-22-frontend-performance.md`](./2026-05-22-frontend-performance.md) | Frontend performance — route caching + image optimization | `/[locale]/[slug]` articles → on-demand ISR (drop `force-dynamic`, `revalidate=60`, `generateStaticParams` → `[]`); the shared `getFamilyTree` compute behind `unstable_cache` (tag `gedcom`); page/note/GEDCOM write routes call `revalidatePath`/`revalidateTag` for instant freshness; `next/image` for portraits. Approach A — no `cacheComponents` migration. [spec](../specs/2026-05-22-frontend-performance-design.md). Shipped 2026-05-22; corrected `revalidateTag('gedcom', 'max')` (Next 16's installed type requires the 2nd `profile` arg, an oversight in the plan text). **Article-ISR task reverted 2026-05-31:** `revalidate=60` 500'd every article page on the auth-on Render deploy — the route read `headers()` during the ISR cache-fill and threw `DYNAMIC_SERVER_USAGE`. Root cause (bisected): `[slug]/loading.tsx`'s *server-side* `useTranslations` — a Suspense fallback can't call `setRequestLocale`, so next-intl fell back to `headers()`. (Not the shared layout: sign-in shares it and renders static.) Route restored to `force-dynamic`; `loading.tsx`/`not-found.tsx` converted to client components (footgun removed — the route *can* now be static, but ~10ms renders don't justify ISR's fragility); `test/render-strategy.test.ts` added as the guard. The other 5 tasks (`getFamilyTree` `unstable_cache`, write-route revalidation, `next/image`) stand. |
| ✅ | [`2026-05-20-git-sync-core.md`](./2026-05-20-git-sync-core.md) | Git sync core plumbing | `push()` + `pullRebase()` + `RebaseConflictError` in `core/src/pages/git.ts`, with a two-clone test fixture. Plan 1 of 3 for the Render deployment ([spec](../specs/2026-05-20-render-deployment-design.md) phase 1). Shipped 2026-05-20 via PR #13 (`130b3bb`) — all 5 tasks; `core/test/pages/git-sync.test.ts` has 6 passing tests. Row flip was missed at merge and corrected 2026-05-20. |
| ✅ | [`2026-05-20-descope-auth.md`](./2026-05-20-descope-auth.md) | Descope auth + write attribution | `WHOAMI_AUTH`-gated Descope login: redirect-or-fall-through `proxy.ts`, `<AuthProvider>`, `/[locale]/sign-in` flow, API self-gating via `requireSession()`, browser writes attributed to the signed-in family member. Plan 2 of 3 for the Render deployment ([spec](../specs/2026-05-20-render-deployment-design.md) phase 2). Shipped 2026-05-20 as `chore:` plumbing — auth stays off by default until plan 3 wires the Render deploy; Task 9 (public `/api/healthz`) was a no-op since the route already existed and the `wai` CLI depends on its `{status,started}` shape. CHANGELOG entry recorded 2026-05-20 once auth was enabled. |
| ✅ | [`2026-05-20-render-deploy-and-sync.md`](./2026-05-20-render-deploy-and-sync.md) | Render deploy + two-way sync | `instrumentation.ts` bootstrap clone, in-process pull/push sync scheduler (`lib/sync.ts`), `REPO_LOCK` write/sync serialisation, Render `family-tree` service config, first deploy + verification, and `SCOPE.md`/`AGENTS.md`/ROADMAP/CHANGELOG reconciliation. Plan 3 of 3 for the Render deployment ([spec](../specs/2026-05-20-render-deployment-design.md) phases 3–5); depends on plans 1 & 2. Shipped 2026-05-20 (P2.20) — all 10 plan tasks; the two-way sync wiring (`lib/sync.ts`, `instrumentation.ts`, `REPO_LOCK` write-path) plus the SCOPE/ROADMAP/CHANGELOG reconciliation. |
| ✅ | [`2026-05-20-auth-account-menu.md`](./2026-05-20-auth-account-menu.md) | Auth account menu | Header-bar account menu for the signed-in family member: an avatar + name trigger opening a popover with email, Descope role(s), relative last-sign-in, and sign-out. Adds a Base-UI `popover` primitive and pure `account-menu-format` helpers. Plus a best-practices pass — a fail-loud `assertAuthConfig` env guard and a fix to the stale "no auth" note in `frontend/AGENTS.md`. Builds on `2026-05-20-descope-auth.md` ([spec](../specs/2026-05-20-auth-account-menu-design.md)). Shipped 2026-05-20 — `feat:` with the CHANGELOG entry; auth-on/off and the fail-loud guard verified by running. |
| 📝 | [`2026-05-20-harden-plan-index-drift-detector.md`](./2026-05-20-harden-plan-index-drift-detector.md) | Harden the plan-index-drift detector | Close a blind spot in `plan-index-drift` check (C): `createdFilesFromPlan()` only matches `` Create: `path` ``, so a plan that declares new files via the File Structure `**create.**` convention (e.g. `git-sync-core`) shipped without (C) catching the stale `🚧` row. Re-base the extraction on the universal File Structure `**create.**` markers. Small, scoped change to `cli/test/plan-index-drift.test.ts`. |
...
**Total: 55 plans** — 47 shipped (✅), 0 in-progress (🚧), 6 sketches (📝), 2 indexes (🗂), 0 abandoned (📦).
