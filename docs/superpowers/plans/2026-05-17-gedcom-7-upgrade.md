...
**Goal:** Upgrade `~/whoami/genealogy/barash-tree.ged` from GEDCOM 5.5.1 to 7.0.18 (released 2026-02-17) and switch the project's parser library from `parse-gedcom@2.0.1` (5.5.1-only) to `gedcom-ts@2026.5.2` (v7-aware, actively maintained). The two changes are coupled — converting the file alone breaks the parser; swapping the parser alone has nothing to parse. Both land on one branch.
