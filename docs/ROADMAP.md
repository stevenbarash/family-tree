...
| **P2.14** Talk-page-translation audit-fold UX. The B.2 real-agent smoke produced two observations worth a follow-up pass on the `.translation.talk.md` layout: (a) **Hierarchy** — talk-page entries land under a `### Talk-page translation` h3 inside `## Unresolved` (commit `40d2c87`). Workable, but the resolve workflow asks the human to tick a checkbox in-place and then *move* the entry under `## Resolved` by hand. The fold should consider whether to either (i) hoist the talk-page entries to a sibling `## Unresolved (talk page)` so the move target is symmetric, or (ii) flatten article + talk entries together with an inline tag prefix (`**[talk]**` / `**[article]**`) and drop the subheading. (b) **Article-translator audit asymmetry** — same smoke run produced 7 substantive entries from `agentTalkTranslator` but ZERO from `agentTranslator` (`pages/ru/aidele.md`'s `## Unresolved` was empty). Either the article prompt's "non-trivial editorial choice" bar is too high or the article translator is silently making calls it shouldn't be. Sample a few other recent `.translation.talk.md` files to see if empty-Unresolved is common; if yes, tighten the article-translation prompt to match the talk-translation prompt's more-explicit examples. | S | CHANGELOG B.2 entry; commits `c77a871`, `40d2c87` |
| **P2.15** Make `frontend/lib/server-services.ts:readTalkBody` locale-aware (Phase B.3 of the talk-page i18n pipeline). Currently locale-blind — always reads `pages/en/<slug>.talk.md`. After B.2 ships, `pages/{ru,uk,he}/<slug>.talk.md` files start landing, but the inline "Editorial discussion" panel on `/ru/<slug>` keeps showing English threads. Read `pages/<locale>/<slug>.talk.md` first; fall back to EN when the localized file is missing. Tests: localized read, fallback to EN, missing-everywhere returns empty. ~20 LOC + tests. | S | CHANGELOG B.2 entry — required to make B.1/B.2 reader-visible |
| **P2.16** Bulk-backfill translated talk pages: 101 EN talk pages × 3 locales (ru, uk, he) = 303 `wai i18n sync` calls. Each call burns one `translate` + one `translate-talk` model invocation. Cost-estimate first against a 5-page sample; if affordable, run as a batch script. Re-syncs are idempotent (the `existingTalkTranslation` field on the request lets the agent preserve human edits), so re-running is safe. Depends on **P2.15** so the translated content has a surface to appear on. | M | CHANGELOG B.2 entry — deployment step for the B.1/B.2 pipeline |
